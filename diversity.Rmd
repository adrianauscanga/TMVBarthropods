...
summary(PCoA)
scatter(PCoA, xax=2, yax=1, posieig="none")
...
ggplot(data= tab, aes(x=A2, y=A1, label=rownames(tab))) + geom_text(size=4, aes(colour = factor(WE))) +          
      ylab(paste0("component 1 explaining ", 
                  round(PCoA$eig[1]/sum(PCoA$eig)*100, 2), "%")) +
      xlab(paste0("component 2 explaining ", 
                  round(PCoA$eig[2]/sum(PCoA$eig)*100, 2), "%")) + 
      scale_colour_discrete(name="TMVB origin") +
      theme_bw()
